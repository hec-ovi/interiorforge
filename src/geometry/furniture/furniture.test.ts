...
describe("shaped furniture", () => {
  ...
  it("puts a piece where it is planned, at its own rotation", () => {
    ...
    const box = bounds(mb);
    expect((box.x[0] + box.x[1]) / 2).toBeCloseTo(item.at[0], 6);
    expect((box.z[0] + box.z[1]) / 2).toBeCloseTo(item.at[1], 6);
    expect(box.x[0]).toBeGreaterThanOrEqual(9.4 - 1e-7);
    expect(box.x[1]).toBeLessThanOrEqual(10.6 + 1e-7);
    expect(box.z[0]).toBeGreaterThanOrEqual(3.6 - 1e-7);
    expect(box.z[1]).toBeLessThanOrEqual(4.4 + 1e-7);
    expect(box.y[0]).toBeCloseTo(3, 6);

    const top = mb.getGroup("cyberpunk/wood/mid")!.positions;
    const topElevation = Math.max(...Array.from({ length: top.length / 3 }, (_, vertex) => top[vertex * 3 + 1]!));
    expect(topElevation).toBeCloseTo(3.75, 6);
    const cornerX = new Set<number>();
    for (let i = 0; i < top.length; i += 3) {
      if (Math.abs(top[i + 1]! - topElevation) < 1e-7) cornerX.add(Math.round(top[i]! * 1e6));
    }
    expect(cornerX.size).toBe(4); // the rectangular top has a seeded, nonzero angular offset

    const rotated = new MeshBuilder();
    emitFurniture(rotated, new MaterialKeys("cyberpunk", "mid"), [{ ...item, rotationDeg: 90 }], makeFrame(0), 3);
    expect(rotated.materials()).toEqual(mb.materials());
    for (const material of mb.materials()) {
      const original = mb.getGroup(material)!.positions, turned = rotated.getGroup(material)!.positions;
      expect(turned.length).toBe(original.length);
      for (let i = 0; i < original.length; i += 3) {
        expect(turned[i]).toBeCloseTo(item.at[0] + original[i + 2]! - item.at[1], 6);
        expect(turned[i + 1]).toBeCloseTo(original[i + 1]!, 6);
        expect(turned[i + 2]).toBeCloseTo(item.at[1] - original[i]! + item.at[0], 6);
      }
    }
  });
