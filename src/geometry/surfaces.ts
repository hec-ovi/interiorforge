...
import { ceilingClear } from "../layout/constants.js";
...
export function buildFloorSurfaces(
  mb: MeshBuilder, keys: MaterialKeys, rooms: RoomPlan[], elevation: number, spaceHeight: number,
  sealedPolys: Point[][],
): void {
  for (const room of rooms) {
    ...
    if (!OPEN_CEILING.has(room.kind)) {
      mb.addHorizontalPolygon(keys.ceiling(), room.polygon, elevation + ceilingClear(spaceHeight), "down");
    }
  }
