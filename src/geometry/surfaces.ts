...
export function buildFloorSurfaces(
  mb: MeshBuilder, keys: MaterialKeys, floor: FloorInterior, spaceHeight: number,
  sealedPolys: Point[][] = [],
): void {
  for (const room of floor.rooms) {
    ...
    if (!OPEN_CEILING.has(room.kind)) {
      mb.addHorizontalPolygon(keys.ceiling(), room.polygon, floor.elevation + ceilingClear(spaceHeight), "down");
    }
  }
