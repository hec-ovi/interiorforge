...
- `nineSlice(width, height, unit = 0.5)` takes metre dimensions and returns fixed corners around fitted plain edge and centre fields: [schema](schema/assembly.schema.json). Two-unit spans close with edge pieces and no centre. Smaller spans use one field. Patterned texture repeats retain their catalog scale.
