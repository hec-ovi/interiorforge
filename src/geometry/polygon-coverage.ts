...
/** Disjoint fragments outside a set of covered polygons, regardless of input winding. */
export class PolygonCoverage {
  private readonly masks: Patch[] = [];

  add(polygon: Point[]): void {
    this.masks.push(...triangles(polygon));
  }

  exposed(polygon: Point[]): Point[][] {
    const bounds = polygonBounds(polygon);
    const masks = this.masks.filter((mask) => rectsOverlap(bounds, mask.bounds));
...
function triangles(polygon: Point[]): Patch[] {
  ...
  if (Math.abs(expected - actual) > Math.max(1e-7, expected * 1e-8)) {
    throw new InteriorError("E_SHELL_BREACH", "surface coverage cannot triangulate the complete floor polygon");
  }
  return parts;
}
...
/** Both polygons are convex. Each edge keeps its outside part and clips the remaining
 * inside against the next edge, producing disjoint convex fragments. */
