...
- `BuildingPlan`
  ...
  - `PlanRoom.polygon`, when present, is the authoritative CCW simple UV footprint inside the wall-depth slab plate; `rect` is its bounds. It supports concave space around an enclosed rectangular service room. Absence retains the outline-clipped rectangular behavior. `PlanDoor.position`, when present, gives the exact UV mounting point on an inset polygon edge; `edge` identifies its outward direction and `at` its along-wall coordinate. Consumers use `doorUvPoint` for every door transform.
  - `room-shape.ts`: `roomPolygon(room, outline?)` resolves that footprint; `roomContains(room, point)` tests it; `roomEdges(room, outline?)` returns directed boundaries with outward `u0/u1/v0/v1` labels, or `edge: null` for diagonal facade clips. `roomCoversRect(room, rect, margin?)` checks complete footprint coverage; `roomAnchor(room)` returns an interior target; `sharedRoomEdges(owner, other, outline?)` returns shared axis-aligned intervals, longest first. Geometry uses these boundaries for surfaces and partitions.
...
- Explicit polygon rooms have axis-aligned interior boundaries; diagonal segments only follow the facade. Their shared boundaries remain fixed during rectangular grid alignment. Navigation, circulation, furniture and lights respect concave notches; wall-mounted items and coves follow real edges.
