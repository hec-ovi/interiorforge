...
export function roomFootprintContains(room: RoomFootprint, point: Point): boolean {
  return boundaryDistance(point, room.polygon) >= -EPS
    && (room.holes ?? []).every(hole => boundaryDistance(point, hole) < -EPS);
}
...
export function roomFootprintAnchor(room: RoomFootprint): Point {
  ...
  if (roomFootprintClearance(room, centroid) > EPS) return centroid;
