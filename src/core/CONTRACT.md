...
- `grid.ts`: `WalkGrid(origin: Point, cellSize, cols, rows)` starts blocked; `forPolygon(outline, cellSize, bounds: Rect)` opens cells whose centers are inside the polygon. Cell size and dimensions must be positive.
  - `forRoomFootprint(footprint: RoomFootprint, cellSize, bounds: Rect) -> WalkGrid` samples the exact `roomFootprintContains` membership at each grid center in the footprint's coordinate frame. Outer boundaries are included and hole interiors and boundaries excluded using the shared `ROOM_FOOTPRINT_EPS = 1e-8` m tolerance. Bounds select grid coverage, not a clipping or snapping change to the footprint.
