 ...
 *  Y up from the surface it stands on. `uv` per face: world faces tile the material at its
 *  published metre size; unit faces wear the whole map once, fitted to the face. */
export class Kit {
  readonly mesh: MeshBuilder;

  constructor(tile: UvScale) {
    this.mesh = new MeshBuilder(undefined, null, tile);
  }

  /** Box from its minimum corner. */
  box(slot: string, [x, y, z]: Vector3, [w, h, d]: Vector3, uv: UvMode = "world", faces?: readonly BoxFace[]): void {
    this.mesh.addBox(slot, { x, z, w, d }, y, y + h, faces, uv);
  }

  /** Box centred in XZ, standing on y. */
  cbox(slot: string, [cx, y, cz]: Vector3, [w, h, d]: Vector3, uv: UvMode = "world", faces?: readonly BoxFace[]): void {
  ...
  cylinder(slot: string, [cx, y, cz]: Vector3, radius: number, height: number, sides = 8, uv: UvMode = "unit"): void {
    const corners: Point[] = [];
    for (let i = 0; i < sides; i++) {
      const a = (i / sides) * Math.PI * 2;
      corners.push([cx + Math.cos(a) * radius, cz + Math.sin(a) * radius]);
    }
    this.mesh.addPrism(slot, corners, y, y + height, uv, "both");
  }
  ...
  rod(slot: string, a: Vector3, b: Vector3, thickness: number): void {
    ...
    for (let i = 0; i < 4; i++) {
      const j = (i + 1) % 4;
      this.mesh.addQuad(slot, [bottom[i]!, top[i]!, top[j]!, bottom[j]!], "unit");
    }
    this.mesh.addQuad(slot, [top[0]!, top[3]!, top[2]!, top[1]!], "unit");
    this.mesh.addQuad(slot, [bottom[0]!, bottom[1]!, bottom[2]!, bottom[3]!], "unit");
  }
  ...
  leaf(slot: string, at: Vector3, headingRad: number, length: number, width: number, lift = 0.25): void {
    ...
    this.mesh.addQuad(slot, [at, left, tip, right], "unit");
    this.mesh.addQuad(slot, [at, right, tip, left], "unit");
  }
