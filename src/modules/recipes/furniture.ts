...
function lens(k: Kit, [cx, cy, cz]: Vector3, length: number, along: "x" | "z" = "x", slot: string = FINISH.lensWarm, section = 0.018): void {
  ...
  k.cbox(slot, [cx, cy - section / 2, cz], size, "unit");
}
...
export const furnitureRecipes: RecipeSet = (add) => {
  ...
  add("fit-reception-desk", (k) => {
    for (const x of [-1.27, 1.27]) k.cbox(FINISH.obsidian, [x, 0, 0], [0.06, 1.06, 0.9], "unit");
    k.cbox(FINISH.obsidian, [0, 0.04, 0.42], [2.48, 1.02, 0.06], "unit");
  ...
  add("fit-kitchen-run", (k) => {
    ...
    slats(k, FINISH.black, 3, [0, 0.1, 0.28], [2.4, 0.81, 0.03], 0.788, "unit");
    ...
    k.cbox(FINISH.black, [-0.55, 0.95, 0], [0.6, 0.01, 0.45], "unit");
    for (const x of [-0.7, -0.4]) for (const z of [-0.1, 0.1]) k.cylinder(FINISH.bronze, [x, 0.96, z], 0.065, 0.008, 6);
    k.cbox(FINISH.chrome, [0.6, 0.93, 0], [0.5, 0.02, 0.4], "unit");
  });
  ...
  add("fit-bed", (k) => {
    ...
    k.cbox(FINISH.glass, [0, 0.65, -0.739], [1.6, 0.74, 0.012], "unit");
  ...
  add("fit-shower", (k) => {
    ...
    k.cbox(FINISH.chrome, [0, 0.055, 0], [0.7, 0.008, 0.04], "unit");
    k.cbox(FINISH.glass, [0.445, 0.06, 0], [0.01, 1.94, 0.9], "unit");
    // the +z panel stops short of the +x end: that gap is the way in
    k.cbox(FINISH.glass, [-0.225, 0.06, 0.445], [0.45, 1.94, 0.01], "unit");
    ...
    k.cbox(FINISH.chrome, [0, 0.9, -0.415], [0.12, 1.1, 0.07], "unit");
    k.cbox(FINISH.chrome, [0, 1.93, -0.32], [0.24, 0.04, 0.26], "unit");
  ...
  add("fit-basin", (k) => {
    ...
    k.cbox(FINISH.chrome, [0, 1.0, -0.215], [0.5, 0.7, 0.02], "unit");
  ...
  add("fit-aquarium-wall", (k) => {
    ...
    k.cbox(FINISH.slate, [0, 0.5, -0.235], [3.0, 1.5, 0.03], "unit");
    ...
    k.cbox(FINISH.glass, [0, 0.5, 0.184], [2.76, 1.35, 0.012], "unit");
  ...
  add("wall-screen", (k) => {
    ...
    k.cbox(FINISH.screen, [0, 0.03, 0.03], [1.14, 0.64, 0.02], "unit");
  });
  ...
  add("wall-art", (k) => {
    ...
    k.cbox(FINISH.art, [0, 0.035, 0.02], [0.63, 0.98, 0.02], "unit");
  ...
  add("fit-capsule-pod", (k) => {
    ...
    for (const x of [-1.2475, 1.2475]) k.cbox(FINISH.hatch, [x, 0.5, 0], [0.005, 1.1, 0.8], "unit");
    k.cbox(FINISH.linen, [0, 0.35, 0.035], [2.3, 0.2, 1.37]);
    // the screen tips back over the sleeper's head
    k.mesh.addQuad(FINISH.screen, [[-0.65, 1.45, -0.5], [-0.65, 1.62, -0.58], [-1.0, 1.62, -0.58], [-1.0, 1.45, -0.5]], "unit");
    for (const x of [-1.195, 1.195]) for (const [y0, y1] of [[0.58, 1.02], [1.12, 1.62]] as const) {
      k.cbox(FINISH.ledCyan, [x, y0, 0.732], [0.04, y1 - y0, 0.04], "unit");
    }
