...
- `fixture.ts`
  - `makeFixture(options) -> { request, shellDoc }`: seeded exterior stand-in: chamfered rectangular outline, per-kind floor heights, entrance plus windows plus balcony doors in the blueprint, low-poly shell GLB (facade skins, floor separator planes, roof). Options: seed, floors, width, depth, building type, tier, theme; all defaulted.
