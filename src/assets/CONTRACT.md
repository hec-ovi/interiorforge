...
- `prepareFurnitureAssets(floors, request, styles, read) -> PreparedFurnitureAssets`: selects and reads at most one shared model per active family (chair, desk, sofa and planter), and publishes the procedural furniture IDs it replaces. Missing or ill-fitting models retain procedural geometry.
...
- `bundled.ts: readBundledAssetModel(asset) -> Promise<Document>`: browser-safe reader for the committed CC0 models through build-time asset URLs.
