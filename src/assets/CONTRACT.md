...
- `prepareFurnitureAssets(floors, request, styles, read) -> PreparedFurnitureAssets`: selects and reads at most one shared model per active family (chair, desk, sofa and planter), and publishes the procedural furniture IDs it replaces. Selection is seed-stable, checks matching local imports before a public model, and falls back to the shipped CC0 model when a local file cannot be read. Missing or ill-fitting models retain procedural geometry.
