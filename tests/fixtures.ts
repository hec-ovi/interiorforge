...
export async function assembly(family: 'mirror-frame' | 'corporate-sectors' = 'mirror-frame'): Promise<InteriorRequest> {
    const entry = new URL('../../exterior/src/index.ts', import.meta.url).href;
    const { planAssembly } = await import(entry);
    const size = family === 'mirror-frame' ? 40 : 56, floors = family === 'mirror-frame' ? 6 : 12;
    const { blueprint } = planAssembly({ buildingId: family, family, seed: 'interior-proof', lot: { width: size, depth: size }, floors });
    return {
