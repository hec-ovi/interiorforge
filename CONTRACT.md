...
- `coreFeasibility(blueprint) -> { fits, mode, bandLength, minCoreLength, minWalkupCoreLength, walkupMaxFloors, maxElevators, crossDepthOk, frameAngleDeg }`: assembler pre-check computed with the exact planCore frame, band and constants. `mode` is `standard` (elevator core), `walkup` (stair-only core for tight footprints; the assembler caps floors at `walkupMaxFloors` and generates) or `none`. Constants and the arithmetic recipe: [schemas/core-feasibility.json](schemas/core-feasibility.json). Elevator demand clamps to the band, so more floors never turn a standard-fitting parcel unfit; error messages quote the same numbers the recipe computes.
